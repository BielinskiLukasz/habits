...
key_files:
  ...
  modified: []
decisions:
  - "APP_VERSION starts at 'v1'; bump per D-10 (only on shell-asset changes)"
...
| File | Purpose |
| --- | --- |
| `js/util/version.js` | Single source of truth for `APP_VERSION = 'v1'` (D-12). Exports as ES module **and** assigns `self.APP_VERSION = APP_VERSION` so the classic-SW context (Plan 02) can read it via `importScripts`. Bumping the version is exactly one edit, in exactly one file. |
...
- Plan 02's `sw.js` derives the cache name as `nawyki-${self.APP_VERSION}` → `nawyki-v1` initially.
...
5. ✅ **Version source-of-truth:** Both `export const APP_VERSION = 'v1'` and `self.APP_VERSION = APP_VERSION` present in `js/util/version.js`.
...
- ✅ `js/util/version.js` exists; contains `export const APP_VERSION = 'v1'` AND `self.APP_VERSION = APP_VERSION`.
