...
decisions:
  ...
  - "Cache name format: `nawyki-${self.APP_VERSION}` — currently resolves to `nawyki-v1`"
...
- Currently resolves to `nawyki-v1` (APP_VERSION = 'v1' per Plan 01-01).
- Bumping `APP_VERSION` to `'v2'` in `js/util/version.js` is the only operation needed to invalidate every cached shell asset; the activate handler will delete `nawyki-v1` on the next SW activation.
...
14. ⏳ Cache Storage shows `nawyki-v1` populated with all 17 SHELL entries.
15. ⏳ Bump APP_VERSION to `'v2'` → reload twice → `nawyki-v1` deleted, `nawyki-v2` active.
