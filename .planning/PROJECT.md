...
| D-28: `APP_VERSION` follows [Semantic Versioning 2.0.0](https://semver.org/) — `MAJOR.MINOR.PATCH` (with optional `-prerelease` / `+build` suffixes). MAJOR = incompatible API/storage-shape changes; MINOR = backwards-compatible feature additions; PATCH = backwards-compatible bug fixes or shell-asset-only changes. The SW cache name derives directly: `nawyki-${APP_VERSION}` (no `v` literal prefix). Starting value `'1.0.0'`. The `v1`/`v2` form used during Phase 1 planning was reclassified as a non-SemVer placeholder and replaced everywhere | Industry-standard versioning scheme; pre-emptively communicates breaking-vs-additive intent to anyone reading the cache name or the diagnostics panel; lets future automation reason about version comparison if ever needed | Locked 2026-05-26; Phase 1 source + docs retro-converted from `'v1'`/`nawyki-v1` to `'1.0.0'`/`nawyki-1.0.0` in a single `chore: switch to semver` commit |
